...
parser = argparse.ArgumentParser(description='Factor transfer student for CIFAR10')
parser.add_argument('--text', default='log.txt', type=str)
parser.add_argument('--exp_name', default='cifar10/s_res56', type=str)
...
def eval(net):
    ...
    for batch_idx, (inputs, targets) in enumerate(loader):
        ...
        loss = criterion_CE(outputs[3], targets).long()
        val_loss += loss.item()
...
def train(teacher, model, epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        tea_outputs = teacher(inputs)
        loss = criterion_CE(outputs[3], targets) * (1 - ALPHA) \
               + nn.KLDivLoss()(nn.functional.log_softmax(outputs[3] / TEMP, dim=1),
                                nn.functional.softmax(tea_outputs[3] / TEMP
                                                      , dim=1)) * ALPHA
...
if __name__ == '__main__':
    ...
    if int(args.log_time):
        folder_name = 'stu_origin_{}'.format(time_log)
    ...
    for epoch in range(RESUME_EPOCH, FINAL_EPOCH + 1):
        ...
        ### Train ###
        train_loss, acc = train(Teacher, model, epoch)
        scheduler.step()
